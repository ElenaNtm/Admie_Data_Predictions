...
#June
df2 = pd.read_excel(url6, sheet_name="System_Production", converters={'(ΣΤΟΙΧΕΙΑ SCADA ΜΗ-ΠΙΣΤΟΠΟΙΗΜΕΝΑ)':str})
substring = 'TOTAL LIGNITE'
...
row = pd.DataFrame([[value]], columns=['Total Lignite in MWh'], index=['2020/07/20210630'])
df_not_found = pd.concat([df_not_found , row])
df_not_found = df_not_found.rename(index={wanted_index[len(wanted_index)-1]: '2020/07/20210630'})  

#July
df2 = pd.read_excel(url7, sheet_name="System_Production", converters={'(ΣΤΟΙΧΕΙΑ SCADA ΜΗ-ΠΙΣΤΟΠΟΙΗΜΕΝΑ)':str})
substring = 'TOTAL LIGNITE'
filtered = df2[df2.apply(lambda row: row.astype(str).str.contains(substring, case=False).any(), axis=1)]
wanted_index = filtered.index.tolist()
...
value = df2.iloc[wanted_index[len(wanted_index)-1],26]
row = pd.DataFrame([[value]], columns=['Total Lignite in MWh'], index=['2020/08/20210731'])
df_not_found = pd.concat([df_not_found , row])
df_not_found = df_not_found.rename(index={wanted_index[len(wanted_index)-1]: '2020/08/20210731'})  
...
df_not_found.tail()
#df_not_found.drop(df_not_found.tail(1).index,inplace=True) 
